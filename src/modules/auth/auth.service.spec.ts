...
describe('AuthService', () => {
  ...
  describe('confirmEmailService', () => {
    it('should throw an error if confirmData.id is missing', async () => {
      const confirmData: ConfirmEmailDto = { userId: undefined };

      await expect(service.verifyEmail(confirmData)).rejects.toThrow(
        new Error(ErrorCode.MISSING_INPUT),
      );
    });

    it('should set isAuthenticated to true and save the user if user exists', async () => {
      const confirmData: ConfirmEmailDto = { userId: 'user-id' };
      ...
      await service.verifyEmail(confirmData);
  ...
  describe('forgotPasswordService', () => {
    it('should throw an error if user is not found', async () => {
      ...
      await expect(service.forgotPassword(forgotPasswordData)).rejects.toThrow(
        new Error(ErrorCode.USER_NOT_FOUND),
      );
    });
    ...
    it('should throw an error if user is not authenticated', async () => {
      ...
      const user = { email: 'user@example.com', isVerified: false };

      jest.spyOn(userRepository, 'findOne').mockResolvedValue(user as User);

      await expect(service.forgotPassword(forgotPasswordData)).rejects.toThrow(
        new Error(ErrorCode.EMAIL_NO_AUTHENTICATED),
      );
    });
    ...
    it('should generate a verification token, cache it, and send a password reset email', async () => {
      ...
      await service.forgotPassword(forgotPasswordData);
  ...
  describe('verifyOTPService', () => {
    it('should verify OTP successfully', async () => {
      ...
      await service.verifyOTP(verifyOtpData);
    ...
    it('should throw an error if OTP is not found in cache', async () => {
      ...
      await expect(service.verifyOTP(verifyOtpData)).rejects.toThrow(
  ...
  describe('resetPasswordService', () => {
    ...
    it('should throw an error if the user is not found', async () => {
      ...
      await expect(service.resetPassword(userData)).rejects.toThrow(
        ErrorCode.USER_NOT_FOUND,
      );
  ...
  describe('freshTokenService', () => {
    it('should generate and return access and refresh tokens for an existing user', async () => {
      ...
      const result = await service.reFreshToken(email);
      expect(mockUserRepository.findOne).toHaveBeenCalledWith({
        where: { email },
      });
    ...
    it('should throw an error if the user does not exist', async () => {
      ...
      await expect(service.reFreshToken(email)).rejects.toThrow(
        ErrorCode.USER_NOT_FOUND,
      );
      expect(mockUserRepository.findOne).toHaveBeenCalledWith({
        where: { email },
